<%@ page import="com.netcracker.students.BatyrkinAndrew.help.Accounts" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
    <title>Hometasks</title>
</head>

<%--<%! public static Accounts accounts = new Accounts(); %>--%>
<body>
<div class="header">
    <h1>Tasks EE</h1>
    <hr>
    <ul class="menu">
        <li>Company - NetCracker</li>
        <li>Student - Batyrkin Andrew</li>
    </ul>
    <hr>
    <h2>   HomeTask #1</h2>
    <h3><a href="tasks/task1/hello_world">         Hello world</a></h3>
    <h3><a href="tasks/task1/login_get.html">      Login (Get)</a></h3>
    <h3><a href="tasks/task1/login_post.html">     Login (Post)</a></h3>
    <h3><a href="tasks/task1/data_and_user-agent"> Data and user-agent</a></h3>
    <h3><a href="tasks/task1/e-mail.html">         Send some e-mail</a></h3>
    <hr>
    <h2>   HomeTask #2 (Sessions, Cookies)</h2>
    <h3><a href="tasks/task2/login_session.html"> Login (Sessions)</a></h3>
    <h3><a href="tasks/task2/login_cookie.html">  Login (Cookies)</a></h3>
    <hr>
</div>
</body>
</html>